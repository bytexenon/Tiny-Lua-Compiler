...
-- You probably shouldn't change the message, as that would require to
-- manually change all expected hash values below.
...
for _ = 1, ITERATIONS do
  for _, alg in ipairs(ALGORITHMS) do
    local func = sha[alg.algorithm]
    local digest = func(MESSAGE)
    assert(digest == alg.expected, alg.algorithm .. " failed")
  end
end
